...
def run_ticket_audit(candidates: list[MarketCandidate], current_bankroll: float):
    ...
    for idx, c in enumerate(candidates, 1):
        ...
        if c.player_name:
            print(f"   Target Giocatore: {c.player_name} (Squadra: {c.team_name or 'N/D'}) | Fixture ID: {c.fixture_id or 'N/D'}")

        rep = pipeline.validate_candidate(c)

        if rep.passed:
            print(f"   🟢 ESITO: APPROVATO (Tutte le Fasi Superate)")
            print(f"   🧠 Sesto Senso:       {c.sixth_sense_analysis}")
            print(f"   📊 Probabilità Reale: {rep.real_probability*100:.1f}% | Fair Odd: @{rep.fair_odds:.2f}")
            ...
            if rep.mathematical_edge != 0.0:
                print(f"   📉 Edge Calcolato:    {rep.mathematical_edge*100:+.1f}% (Fair Odd: @{rep.fair_odds:.2f})")
    ...
    if len(approved_candidates) < len(candidates):
        print(f"🚨 ATTENZIONE: {len(candidates) - len(approved_candidates)} selezioni sono state SCARTATE perché non conformi alle regole.")
    ...
    if approved_candidates:
        ...
        print(f"• Stake Consigliato (Kelly Frax): €{recommended_stake:.2f} ({recommended_stake/max(1, current_bankroll)*100:.1f}% del bankroll)")
...
def main():
    parser = argparse.ArgumentParser(description="Audit Pre-Schedina con Protocollo Matematico & Sesto Senso")
    ...
    if args.demo:
        demo_candidates = [
            MarketCandidate(
                ...
                bookmaker_odd=1.31,
                estimated_p_90=0.88,
                sixth_sense_analysis="PSV con forte spinta interna al Philips Stadion ma Shakhtar esperto e resiliente. Il mercato 1X+Over 1.5 protegge dall'1-1 e sfrutta le transizioni olandesi."
            ),
            MarketCandidate(
                ...
                bookmaker_odd=1.30,
                estimated_p_90=0.84,
                sixth_sense_analysis="Roma di Gasperini con assetto propositivo (Malen, Dybala, Soule), produce sempre occasioni ma in trasferta a Istanbul non dilaga oltre i 3 gol."
            ),
            MarketCandidate(
                ...
                bookmaker_odd=1.32,
                estimated_p_90=0.83,
                sixth_sense_analysis="Entusiasmo storico al Sinigaglia per l'esordio europeo del Como. Il Lipsia concede spazi in contropiede ma il Como segna solitamente 1-2 reti."
            ),
            MarketCandidate(
                ...
                sixth_sense_analysis="Bayern con tendenza ad avvio diesel contro blocchi bassi scandinavi. Rischio 0-0 al 45' altissimo.",
            ...
            MarketCandidate(
                ...
                is_compound_time_market=True,
                is_intermediate_deadline=True,
                # Intenzionalmente senza sesto senso per testare il blocco di Fase 4
                sixth_sense_analysis=""
            )
        ]
        run_ticket_audit(demo_candidates, args.bankroll)
    elif args.file:
        p = Path(args.file)
        if not p.exists():
        ...
        run_ticket_audit(candidates, args.bankroll)
    else:
        parser.print_help()
